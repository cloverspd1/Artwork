...
function BindUserTags(selector) {
    ...
    $(selector + ".user-tags").each(function () {
        ...
        var min_chars = ele.attr("min-chars"); //Rushit
        ...
        if (typeof (min_chars) === "undefined" || min_chars == null || min_chars == '') {
            min_chars = 2;
        }
...
function BindFileUploadControl(options) {
	...
	*/
    ...
    if (typeof options.fileSize !== "undefined" && options.fileSize !== null) {
        fileSize = (options.fileSize) ? options.fileSize : (5 * 1024 * 1024);
    }
    ...
    var acrionURL = "";
    if (options.Url.toLowerCase() == 'addupdateuploadfile')
        acrionURL = BASEPATHURL + "/ViewArtwork/" + options.Url;
    ...
    var uploader = uploader = new qq.FileUploader(
    {
        ...
        multiple: mutilple,
        debug: true,
        sizeLimit: fileSize, //5 * 1024 * 1024, // 5 MB
                ...
                if (typeof (responseJSON.IndividualUpload) !== "undefined" && responseJSON.IndividualUpload) {
                    if (responseJSON.Status) {
                        if (typeof options.CallBack !== "undefined" && options.CallBack !== null) {
                            ...
                            //eval(options.CallBack + "(responseJSON, options.ElementId)");
...
function onAjaxError(xhr) {
    if (!UserAborted(xhr)) {
        if (xhr.status == 403) {
            //AlertModal(getMessage("Error", "Your session has been expired, please login again.", BootstrapDialog.TYPE_DANGER, function () {
...
function adjustFrameSize(contentHeight) {
    ...
    if (args.length < 2 && document.referrer) {
        args = document.referrer.split("?");
        params = decodeURIComponent(args[1]).split("&");
    }
    else {
        params = args[1].split("&");
    }
...
//Dynamic File Attachment Control
function BindAttachment(eleFNL, divid, maxFileupload, artWorkTypeCode, fileSize) {

    if (typeof (maxFileupload) === 'undefined') maxFileupload = 10;
    var strParameter = '';
    var uploadfileURL = "UploadFile";
    if (maxFileupload == 1) {
        uploadfileURL = "UploadFileWithRename";
        // strParameter = "modelName:'" + $('#ModelName').val() + "',artworkTypeCode:'" + artWorkTypeCode + "'"
    }
    else if (maxFileupload == 2) {
        uploadfileURL = "AddUpdateUploadFile";
        maxFileupload = 1;
    }
    else {
        strParameter = '';
    ...
    if ($("#" + eleFNL).length != 0) {
        if (window[divid] == "undefined" || window[divid] == null || window[divid] == '') {
            window[divid] = [];
        }

        BindFileUploadControl({
            ElementId: divid, Params: { artworkTypeCode: artWorkTypeCode }, Url: uploadfileURL, fileSize: fileSize,
            AllowedExtensions: [],
            MultipleFiles: false,
            maxFiles: maxFileupload, //Changes for number of files upload per control and default is 10 files
            CallBack: function (result) {
                window[divid].push(result);
                $("#" + eleFNL).val(JSON.stringify(window[divid])).blur();
            }
        });

        window[divid] = BindFileList(eleFNL, divid);
    }
}

function RemoveImage(ele) {
    var divid = $(ele).parents(".qq-uploader").parent().attr("id");
    var eleFNL = $("#" + divid).next("input").attr("id")
    var Id = $(ele).attr("data-id");
    var li = $(ele).parents("li.qq-upload-success");
    var itemIdx = li.index();
    ConfirmationDailog({
        title: "Remove", message: "Are you sure to remove file?", id: Id, url: "/NewArtwork/RemoveUploadFile", okCallback: function (id, data) {
            li.find(".qq-upload-status-text").remove();
            $('<span class="qq-upload-spinner"></span>').appendTo(li);
            li.removeClass("qq-upload-success");
            var idx = -1;
            var tmpList = [];
            $(window[divid]).each(function (i, item) {
            ...
            if (idx >= 0) {
                window[divid] = tmpList;
                li.remove();
                if (window[divid].length == 0) {
                    if (/^[a-zA-Z0-9- ]*$/.test(eleFNL) == false) {
                        eleFNL = eleFNL.replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, "\\$&");
                    }
                    $("#" + eleFNL).val("").blur();
                } else {
                    if (/^[a-zA-Z0-9- ]*$/.test(eleFNL) == false) {
                        eleFNL = eleFNL.replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, "\\$&");
                        $('input[name="' + eleFNL + '"]').val(JSON.stringify(window[divid])).blur();
                    }
                    else {
                        $("#" + eleFNL).val(JSON.stringify(window[divid])).blur();
                    }
