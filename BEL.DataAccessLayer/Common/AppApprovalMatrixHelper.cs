            ...
            if (items != null && items.Count != 0)
            {
                foreach (ListItem item in items)
                {
                    slamatrix.Add(new SLAMatrix()
                    {
                        ...
                        IsAutoRejection = item["IsAutoRejection"] != null ? (bool)item["IsAutoRejection"] : false,
                    ...
                    if (approvers != null && approvers.Count != 0 && !string.IsNullOrEmpty(userEmail))
                    {
                        ...
                        switch (actionPerformed)
                        {
                            ...
                            case ButtonActionStatus.Hold:
                                Logger.Info("Hold condition => any approver=" + approvers.Any(p => p.Levels == currLevel.ToString()).ToString());
                                if (approvers.Any(p => p.Levels == currLevel.ToString()))
                                {
                                    approvers.ForEach(p =>
                                    {
                            ...
                            case ButtonActionStatus.Resume:
                                Logger.Info("Resume condition => any approver=" + approvers.Any(p => p.Levels == currLevel.ToString()).ToString());
                                if (approvers.Any(p => p.Levels == currLevel.ToString()))
                                {
                                    approvers.ForEach(p =>
                                    {
