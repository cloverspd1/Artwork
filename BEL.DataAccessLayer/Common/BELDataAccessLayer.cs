                ...
                if (!string.IsNullOrEmpty(approverMatrixListName))
                {
                    ...
                    else
                    {
                        if (actionPerformed == ButtonActionStatus.NextApproval || actionPerformed == ButtonActionStatus.Delegate)
                        {
                            ...
                            {
                                ////Added for parallel case completion, if next level of approver is not present then workflow will complete and mail will send to respective users
                                actionPerformed = actualSection.ActionStatus = ButtonActionStatus.Complete;
                            }
            ...
            switch (actionPerformed)
            {
                ...
                case ButtonActionStatus.NextApproval:
                    if (approversDataFromList != null && approversDataFromList.Count != 0)
                    {
                        ...
                        to = allToUsers.Trim(',');
                        if (!approversDataFromList.Any(q => Convert.ToInt32(q.Levels) == nextLevel && !string.IsNullOrEmpty(q.Approver) && q.Status == ApproverStatus.APPROVED))
                        {
                            cc = approversDataFromList.Where(p => p.Role == UserRoles.CREATOR).First().Approver;
                        }
                        role = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == currLevel).Role;
                        mailCustomValues[Parameter.DUEDATE] = approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == nextLevel).DueDate.HasValue ? approversDataFromList.FirstOrDefault(p => Convert.ToInt32(p.Levels) == nextLevel).DueDate.GetValueOrDefault().ToString("dd/MM/yyyy") : "";
                    ...
                    foreach (PropertyInfo property in properties)
                    {
                        ...
                        else if (property.GetCustomAttribute<IsTranAttribute>() != null && property.GetCustomAttribute<IsTranAttribute>().IsTranField)
                        {
                            if (listDetail.IsMainList)
                            {
                                ...
                                if (!string.IsNullOrEmpty(listName) && tranType != null && !tranList.ContainsKey(listName))
                                {
        ...
        public ISection GetActivityLogUsingCSOM(ClientContext context, Web web, string listName, int lookupId, bool isActive)
        {
            ...
            if (context != null && web != null && !string.IsNullOrEmpty(listName) && lookupId > 0)
            {
                ...
                foreach (ListItem item in items)
                {
                    ...
                    log.PerformedOn = Convert.ToDateTime(item["ActivityDate"]);
                    log.Created = Convert.ToDateTime(item["Created"]);
