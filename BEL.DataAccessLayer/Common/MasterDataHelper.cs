    ...
    public class MasterDataHelper : BELDataAccessLayer
    {
        ...
        private IMaster GetMasterDataOf(ClientContext context, Web web, IMaster master)
        {
            ...
            if (objMaster == null)
            {
                ...
                ListItemCollectionPosition itemPosition = null;   ////Add For Extend Treshold limit of Sharepoint online (5000 items in single call).
                dynamic tempImaster = Activator.CreateInstance(master.GetType()) as IMaster; ;
                objMaster.Items = new List<IMasterItem>();
                while (true)
                {
                    CamlQuery camlQuery = new CamlQuery();
                    camlQuery.ListItemCollectionPosition = itemPosition;
                    camlQuery.ViewXml = @"<View><Query></Query>" + viewFields + "<RowLimit>1000</RowLimit></View>";

                    ListItemCollection items = spList.GetItems(camlQuery);
                    context.Load(items);
                    context.ExecuteQuery();
                    itemPosition = items.ListItemCollectionPosition;
                    tempImaster = this.AssignPropertyValues(tempImaster, context, web, items, itemType);
                    objMaster.Items.AddRange(tempImaster.Items);
        ...
        /// </returns>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity", Justification = "NA")]
        public IMaster AssignPropertyValues(dynamic objMaster, ClientContext context, Web web, ListItemCollection items, Type itemType)
        {
                    ...
                    if (property.GetCustomAttribute<FieldColumnNameAttribute>() != null && !string.IsNullOrEmpty(property.GetCustomAttribute<FieldColumnNameAttribute>().FieldsInformation))
                    {
                        ...
                        else if (isFile)
                        {
                            if (Convert.ToString(item["Attachments"]) == "True")
                            {
                                ...
                                property.SetValue(listItem, objAttachmentFiles);
                            }
                        }
                        else
                        {
